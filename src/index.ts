...
const callRebuildWebHook = async () => {
  try {
    const REBUILD_HOOK = process.env.DEPLOY_HOOK_URL;

    if (!REBUILD_HOOK) return console.error("No rebuild hook URL provided!");

    const response = await fetch(`${REBUILD_HOOK}`, {
      method: "POST",
    });

    if (!response.ok) {
      console.error("Triggering deploy hook failed: ", await response.text());
    } else {
      console.log("Deploy hook triggered successfully!");
    }
  } catch (err) {
    console.error("Erro triggering build hook: ", err);
  }
};
...
app.post("/api/posts", async (c) => {
  ...
  try {
    ...
    await callRebuildWebHook(); // Calling the hook to rebuild the FE.
